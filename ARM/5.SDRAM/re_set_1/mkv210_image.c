...
int main (int argc, char *argv[])
{
	...
	if (argc != 3)
	{
		...
		return -1;				//应该表示的是有错误的意思，好像也没什么用吧
	}

	// 2. 分配16K的buffer
	BufLen = BUFSIZE;
	Buf = (char *)malloc(BufLen);			//分配函数
	if (!Buf)
	{
	...
	memset(Buf, 0x00, BufLen);			//填充值进去，就是0

	// 3. 读源bin到buffer
	// 3.1 打开源bin
	fp = fopen(argv[1], "rb");			//fopen函数用于打开文件, 其调用格式为: FILE *fopen(char *filename, *type); 
	if( fp == NULL)
	{
	...
	// 3.2 获取源bin长度，fseek函数也可以这样理解,相当于在文件当中定位
	//  函数 ftell() 用于得到文件位置指针当前位置相对于文件首的偏移字节数
	fseek(fp, 0L, SEEK_END);								// 定位到文件尾
	fileLen = ftell(fp);									// 得到文件长度函数
