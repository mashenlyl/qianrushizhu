...
#include <linux/device.h>		// class_create和device_create函数
...
dev_t major = -1;
char kbuf[100];
// 这里使用指针的方式来替代cdev结构体变量的方式，从而可以使用cdev_alloc函数来分配内存，更方便的管理内存
//static struct cdev test_cdev;
static struct cdev* pTest_cdev;
...
*/
	...
	// 这里的cdev_alloc函数相当于是分配了一份cdev结构体类型的内存，然后返回这份内存的地址
	pTest_cdev = cdev_alloc();
	// 第二步：cdev结构体初始化，把cdev结构体赋值，还有和file_operations结构体绑定在一起
	cdev_init(pTest_cdev, &test_fops);
	
	// 第三步：注册驱动
	retval = cdev_add(pTest_cdev, major, TEST_NUM_MINORS);
	if (retval)
		goto flag2;
	
	// printk(KERN_INFO "GPJ0CON = %x\n", rGPJ0CON);

	// 自动创建设备文件的，这里的最后一个参数是sys目录下的类文件夹名
	test_class = class_create(THIS_MODULE, "lyl_class");

	if (IS_ERR(test_class)) {
		printk(KERN_ERR "lyl_class:" "Could not register class 'lyl_class'\n");
		return 0;
	}

	// 这里的最后一个参数是dev目录下的设备文件
	device_create(test_class, NULL, major, NULL, "test");
